...
void FAnimNode_FootPlacementIK::PreUpdate(const UAnimInstance* InAnimInstance)
{
	...
	MeshRefPose.InitPose(&InAnimInstance->GetRequiredBones());

	CalculateEffector(InAnimInstance->GetWorld()->GetDeltaSeconds(), InAnimInstance->GetSkelMeshComponent(), MeshRefPose);
}
...
void FAnimNode_FootPlacementIK::CalculateEffector(float DeltaTime, USkeletalMeshComponent* SkelComp, FCSPose<FCompactPose>& MeshBases)
{
	...
	FAnimationRuntime::ConvertCSTransformToBoneSpace(SkelComp->GetComponentToWorld(), MeshBases, EndBoneWorldTransform, FCompactPoseBoneIndex(INDEX_NONE), BCS_WorldSpace);
...
void FAnimNode_FootPlacementIK::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	const FCompactPoseBoneIndex LowerLimbIndex = Output.Pose.GetPose().GetParentBoneIndex(IKBoneIndex);
	...
	const FCompactPoseBoneIndex UpperLimbIndex = Output.Pose.GetPose().GetParentBoneIndex(LowerLimbIndex);
	...
	const FTransform EndBoneLocalTransform = Output.Pose.GetLocalSpaceTransform(IKBoneIndex);
	...
	FTransform EndBoneCSTransform = Output.Pose.GetComponentSpaceTransform(IKBoneIndex);
	...
	FAnimationRuntime::ConvertBoneSpaceTransformToCS(Output.AnimInstanceProxy->GetComponentTransform(), Output.Pose, EffectorTransform, FCompactPoseBoneIndex(INDEX_NONE), BCS_WorldSpace);
